...
#define KERNEL_CODE_END             (KERNEL_BASE + 0x00600000UL)  // 3GB + 6MiB (1MiB reserved before)
...
#define DYLIB_REGION_VIRT_START     (KERNEL_BASE + 0x00100000UL)  // 3GB + 1MiB virtual
...
static inline int is_video_memory_address(uintptr_t addr) {
    return addr >= VIDEO_MEMORY_VIRT && addr < (VIDEO_MEMORY_VIRT + VIDEO_BUFFER_SIZE);
}
