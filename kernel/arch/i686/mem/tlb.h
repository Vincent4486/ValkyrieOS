 ...
 * 
 * The TLB is a hardware cache that speeds up virtual-to-physical address translation.
 * When the CPU needs to translate a virtual address, it first checks the TLB:
 *  - TLB hit: Fast translation (nanoseconds)
 *  - TLB miss: Slow page table walk (hundreds of nanoseconds), then caches in TLB
 * 
 ...
 */
static inline void tlb_invalidate_range(uintptr_t vaddr_start, uintptr_t vaddr_end) {
 ...
 * 
 * This switches the active page directory and automatically invalidates all TLB entries.
