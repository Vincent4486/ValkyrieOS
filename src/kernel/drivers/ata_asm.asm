...
.r_don:
	; Convert return value: ZF set = success (0), CF set = failure (-1)
	jnz .ret_error
	xor eax, eax            ; eax = 0 (success)
	jmp .ret_done
.ret_error:
	mov eax, -1             ; eax = -1 (failure)
.ret_done:
	pop edi
	pop esi
	pop edx
...
; do a singletasking PIO ata "software reset"
; cdecl calling convention (32-bit x86):
;   [esp+4] = dcr_port (alternate status / device control register port)
global srst_ata_st
srst_ata_st:
	push ebp
	mov ebp, esp
