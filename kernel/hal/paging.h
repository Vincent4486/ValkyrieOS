...
#define HAL_ARCH_Paging_GetCurrentPageDirectory i686_Paging_GetCurrentPageDirectory
...
static inline void HAL_Paging_Initialize(void){
    HAL_ARCH_Paging_Initialize();
}

static inline void HAL_Paging_Enable(void){
    HAL_ARCH_Paging_Enable();
}
...
static inline bool HAL_Paging_MapPage(void *page_dir, uint32_t vaddr, uint32_t paddr, uint32_t flags){
...
static inline uint32_t HAL_Paging_GetPhysicalAddress(void *page_dir, uint32_t vaddr){
...
static inline void HAL_Paging_PageFaultHandler(uint32_t fault_address, uint32_t error_code){
...
static inline void HAL_Paging_FlushTlb(void){
    HAL_ARCH_Paging_FlushTlb();
}
...
static inline void HAL_Paging_SelfTest(void){
    HAL_ARCH_Paging_SelfTest();
}
